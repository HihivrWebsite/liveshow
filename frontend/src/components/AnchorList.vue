...
export default {
  ...
  setup() {
    ...
    const onSelectionChange = (selected) => {
      ...
      window.dispatchEvent(new CustomEvent('drawer-selection-change', { detail: { count: selected.length } }))
    }
      ...
      try {
        ...
        const htmlContent = `
          <div style="font-family:'Microsoft YaHei',sans-serif;background:#FFF8E1;padding:20px;width:1200px;">
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;"><span style="color:#f9729a;font-weight:bold;font-size:1.1rem;">${startLabel}-${endLabel}数据</span><span style="color:#00BFFF;font-weight:bold;font-size:1.1rem;">向阳Hihi粉丝站-维阿PSP斗虫榜 dc.hihivr.top</span></div>
    ...
    const getWhaleClass = (percent) => {
      if (percent < 10) return 'whale-green'
      if (percent < 30) return 'whale-yellow'
      if (percent < 50) return 'whale-orange'
      return 'whale-red'
    }
